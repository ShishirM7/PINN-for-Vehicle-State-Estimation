...
# Load Simulink data
data = scipy.io.loadmat('C:\\CM_Projects\\SSA_SM\\src_cm4sl\\SM_PY_import\\Simulink simulation output for PINN training\\Track 1_Base Track\\DSchramm_FrontAxle_ext.mat')
...
delta = data['delta'].squeeze().astype(np.float32)  # ← Must be present
...
def physics_loss(beta_pred, v, psi_dot, delta, l_v, alpha_v):
    num = v * torch.sin(beta_pred) + l_v * psi_dot
    denom = v * torch.cos(beta_pred)
    tan_term = num / denom
    
    alpha_v_est = delta - torch.atan(tan_term)
    
    return torch.mean((alpha_v_est - alpha_v) ** 2)
'''
def physics_loss(beta_pred, v, psi_dot, delta, l_v, alpha_v):
    eps = 1e-6
    tan_term = torch.tan(delta - alpha_v).clamp(-np.pi/2 + eps, np.pi/2 - eps)
    num = l_v * psi_dot + v * tan_term
    denom = torch.sqrt(v**2 + num**2)
    
    beta_th = torch.atan2(num, v)
...
for epoch in range(epochs):
    ...
    loss_phy = physics_loss(beta_pred, v_tensor, psi_dot_tensor, delta_tensor, l_v, alpha_v_tensor)
...
with torch.no_grad():
    ...
    tan_term = torch.tan(delta_tensor - alpha_v_tensor)
    num = l_v * psi_dot_tensor + v_tensor * tan_term
    beta_th_tensor = torch.atan2(num, v_tensor)
    beta_th = beta_th_tensor.cpu().numpy()
...
traced.save("Schramm_FrontAxle_ext.pt")
