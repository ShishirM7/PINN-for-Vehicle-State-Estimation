...
# Load Simulink data
data = scipy.io.loadmat('C:\\CM_Projects\\SSA_SM\\src_cm4sl\\SM_PY_import\\Simulink simulation output for PINN training\\Track 1_Base Track\\KTakikawa.mat')
...
'''
# Prepare full input with constants
N = X.shape[0]
const_features = np.tile([m, L_f, L_r, L], (N, 1)).astype(np.float32)  # shape [N, 4]
X_full = np.hstack([X, const_features])  # shape [N, 7]
...
def physics_loss(pred, v, psi_dot, K_r):
    v_safe = torch.sign(v) * torch.clamp(torch.abs(v), min=1e-3)  # avoid division by zero
...
K_r_tensor = K_r_tensor.unsqueeze(1)       
...
'''
N = v_tensor.shape[0]  # or manually set N = 73541

x = torch.cat([
    v_tensor,
    psi_dot_tensor,
    K_r_tensor,
    torch.full((N, 1), m, dtype=torch.float32),
    torch.full((N, 1), L_f, dtype=torch.float32),
    torch.full((N, 1), L_r, dtype=torch.float32),
    torch.full((N, 1), L, dtype=torch.float32)
], dim=1)
'''
torch.onnx.export(
    model,
...
traced.save("K_Takikawa.pt")

from scipy.io import savemat
savemat("const_KT.mat", {
